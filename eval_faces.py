...
import utils
from utils import str2bool, purify_faces, get_image_classifier, load_data
...
class ResNet_Adv_Model(nn.Module):
    def __init__(self, args, config):
        super().__init__()
        # image classifier
        self.resnet = get_image_classifier(
...
class SDE_Adv_Model(nn.Module):
    def __init__(self, args, config):
        ...
        if args.diffusion_type == 'ddpm':
            ...
            self.runner = Diffusion(args, config, device=config.device)  # ddpm
        else:
    ...
    def purify(self, x):
        ...
        print("running image_editing_sample...")
...
def eval_bpda(args, config, model, x_val, y_val, adv_batch_size, log_dir):
    ...
    purify_faces(model, x_val, bs=adv_batch_size)
    # print('initial accuracy: {:.2%}, time elapsed: {:.2f}s'.format(
    #     init_acc, time.time() - start_time))
...
def parse_args_and_config():
    ...
    parser.add_argument('--diffusion_type', type=str,
                        default='ddpm', help='[ddpm, sde, celebahq-ddpm]')
